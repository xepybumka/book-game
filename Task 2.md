...
* Работоспособная модель:
    ...
    ~~+ Переход на параграфы посредством метода POST или альтернативным методом~~
    ~~+ Изменить параграфы: вместо text будет html.~~
    + Тестами покрыть административную панель.
    + Тестами покрыть игровую часть.
    + Создать сущность "События" которая привязывается к параграфу. 
      От вида события на контентной части параграфов появляется кнопка "Начать бой",
      "Проверить удачу", "Торговать". Будет соответствующий вызов события, что заменит контентную часть.
    + Ведение статистики для вывода после игры: кол-во нанесенного урона, потраченных монет и прочего.
    + Тестовый сценарий:
      + Создание персонажа.
      + Переход на параграф.
      + Подбор предмета в сумку.
      + Персонаж мертв от подбора предмета.
    ...
    + Добавить скрытые параметры для проверки внутреигровых действий: наличие предмета, убийство противников и т.д.
    + Административная часть.
      + Авторизация в административной панели
      + Изоляция в административной панели по авторизации
    + Тестовый контур (режим бога/админа).
...
* Адаптивная верстка для браузеров: переделать фиксированные размеры в px на rem
